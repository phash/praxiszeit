...
|-------|---------|-----------|--------------------|
| **Wochenarbeitszeit** (`weekly_hours`) | Stunden/Woche | Vertraglich vereinbarte Stunden pro Woche (z. B. 40,0). Historisch korrekt über `WorkingHoursChange`. Bei Tagesplan die **Summe** der Tageswerte (serverseitig abgeleitet). | `get_weekly_hours_for_date()` (NIE `user.weekly_hours` direkt lesen) |
| **work_days_per_week** | Tage/Woche | Anzahl Arbeitstage pro Woche (z. B. 5). Seit #431 ebenfalls historisiert — bei Tagesplan aus der Zahl der Wochentage mit Stunden abgeleitet, nicht frei wählbar. | `get_schedule_for_date().work_days_per_week` (NIE `User.work_days_per_week` direkt für ein Datum lesen) |
