...
**Windows-Paket im Emulator testen:** Ein echtes Windows 11 (dockur/windows, KVM) installiert das gebaute `windows-x64.zip` unbeaufsichtigt, startet den Dienst, meldet sich an und schreibt das Ergebnis zurueck — inklusive Rechte-Nachweis auf `praxiszeit.conf`/`ssl/key.pem` und der Selbstreparatur beim Dienststart. Harness in `tools/win-test/`, Anleitung: [WINDOWS-EMULATOR-TEST.md](WINDOWS-EMULATOR-TEST.md). Unit-Tests und `validate-release.sh` sehen von alldem nichts.
