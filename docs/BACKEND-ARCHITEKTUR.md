...
> **Tech-Debt — `half_day`-Sondertage bei Betriebsferien:** Generierte Betriebsferien-Absences sind immer **volle Tage** (`half_day=False`). Fällt ein als `half_day` konfigurierter Sondertag (24./31.12.) in eine Schließung, wird er dort aktuell als **voller Tag** gebucht (kein Halbtags-Split innerhalb der Closure-Buchung; betrifft auch den #314-Überstunden-Split). Bei direkter/antragsbasierter Buchung greift der Halbtag dagegen über `half_day=True`.
