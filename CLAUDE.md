...
**`dist/…-SHA256SUMS.txt`-Self-Reference (behoben):** früher erfasste der `praxiszeit-<ver>-*`-Glob eine **SHA-Datei aus einem vorherigen Build** derselben Version mit → die SHA-Datei listete sich selbst → `sha256sum -c` meldete ein kosmetisches `FAILED` für die SHA-Datei. Der Build entfernt die alte SHA-Datei jetzt VOR dem Glob und schreibt über eine Temp-Datei + `mv` (Glob trifft nur die echten Artefakte). Bei Änderungen an Schritt 7 (`build-release.sh` ~Z. 1042) beibehalten.
