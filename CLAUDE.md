...
**⚠️ TODO (nächstes Release, beschlossen 2026-06-16) — `windows-x64.zip` ist ~2× zu groß durch eingebettete `setup.exe`:** Phase 5b kopiert die fertige `setup.exe` (522 MB) zurück in `WIN_DIR` (`build-release.sh` ~Z.697) BEVOR `windows-x64.zip` aus `WIN_DIR` gepackt wird → das ZIP enthält das Paket **doppelt** (loser .bat-Fallback-Baum **+** die Single-File-`setup.exe`, die ihrerseits das ganze Paket komprimiert eingebettet hat) = 985 MB statt ~460 MB. Kompression bringt kaum was: ~92 % des ZIPs sind bereits gepackte Binaries (`setup.exe` 546 MB + `postgresql-installer.exe` 372 MB + `vc_redist` 25 MB, unkomprimiert gesamt 1,23 GB → ZIP 985 MB ≈ Ratio 1,2×). **Geplanter Fix:** `setup.exe` nur als Standalone-`dist/`-Artefakt ausgeben, NICHT mit in den ZIP-Baum kopieren (Z.697-`cp … ${WIN_DIR}/setup.exe` entfernen). ⚠️ **Trade-off vor Umsetzung klären:** Der empfohlene Install-Weg (Build-Zusammenfassung + [docs/INSTALL-NATIVE.md](docs/INSTALL-NATIVE.md): „ZIP entpacken → setup.exe doppelklicken") setzt die `setup.exe` IM ZIP voraus → bei der Umstellung die Empfehlungs-Doku auf den **Standalone-`setup.exe`-Download** umlenken (oder ZIP-Nutzer auf den `.bat`-Fallback verweisen).
