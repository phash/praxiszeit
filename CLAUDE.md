...
**`dist/…-SHA256SUMS.txt` listet sich selbst** → `sha256sum -c <datei>` meldet 1 (kosmetisches) `FAILED` für die SHA-Datei selbst; die 6 Artefakt-Hashes stimmen trotzdem. (Optionaler Build-Script-Cleanup: die SHA-Datei aus ihrer eigenen Auflistung ausschließen.)
